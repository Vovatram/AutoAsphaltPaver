...
_ALL_VEHICLES = _CLOSURE_VEHICLES + _VEHICLES_P1 + _VEHICLES_P2 + _VEHICLES_F1 + _VEHICLES_F3
...
@app.post("/api/repair/check")
def check_repair_plan(req: RepairPlanRequest):
    road = next((r for r in ROADS if r["id"] == req.road_id), None)
    if not road:
        raise HTTPException(status_code=404, detail="Road not found")
    ...
    return check_repair(
        road=road, lane=lane, window_str=req.window,
        all_vehicles=_ALL_VEHICLES, factories=FACTORIES,
    )
