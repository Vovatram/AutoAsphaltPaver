...
function ParkingView({ parking, onSelectVehicle }) {
  return (
    <>
      <div className="sticky top-0 bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between shrink-0 z-10">
        <div>
          <h2 className="font-bold text-gray-900 text-sm">{parking.name}</h2>
          <p className="text-xs text-gray-400 mt-0.5">Техника на стоянке</p>
        </div>
      </div>
...
function VehicleView({ vehicle, onClose }) {
...
export default function PanelVehicle({ panel, dark, onClose, onSelectVehicle }) {
  return (
    <div className="absolute top-0 right-0 h-full w-96 bg-white shadow-2xl overflow-y-auto z-10 flex flex-col">
      {panel.type === 'parking' ? (
        <>
          {/* Parking header needs its own close button */}
          <div className="sticky top-0 bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between shrink-0 z-10">
            <div>
              <h2 className="font-bold text-gray-900 text-sm">{panel.data.name}</h2>
              <p className="text-xs text-gray-400 mt-0.5">Техника на стоянке</p>
            </div>
            <button
              onClick={onClose}
        ...
        <VehicleView vehicle={panel.data} onClose={onClose} />
      )}
    </div>
  );
}
