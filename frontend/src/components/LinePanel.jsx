    ...
    <div className="absolute inset-0 z-20 bg-black/40 flex items-center justify-center p-6">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          {/* Weather */}
          <div className={`rounded-xl p-3.5 border ${road.weather_suitable ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5">
              Погодные условия
            </p>
            <div className="flex items-center gap-2">
              <span className="text-lg">{road.weather_suitable ? '✅' : '⛔'}</span>
              <p className={`text-sm font-semibold ${road.weather_suitable ? 'text-green-700' : 'text-red-700'}`}>
                {road.weather_suitable ? 'Укладка возможна' : 'Укладка не рекомендуется'}
              </p>
            </div>
            <p className="text-xs text-gray-600 mt-1">{road.weather_note}</p>
          </div>

          {/* Action */}
          {road.weather_suitable && (
            <button
              onClick={onStartPlan}
